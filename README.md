...
这是一个基于 ESP32-WROOM-32E 开发板的无线串口调试助手。ESP32 连接被测设备的 TTL UART，PC/手机在同一局域网打开 ESP32 的 IP 地址，即可通过网页收发串口数据。
...
- 只允许直接连接 `3.3V TTL`。如果被测设备是 `5V TTL`，建议加电平转换。
- 不能把 RS232 的正负电压信号直接接到 ESP32。后续做 RS232 可加 `MAX3232`；做 RS485/RS422 可加对应差分收发器。
- 淘宝常见 YD-ESP32 / ESP32-WROOM-32E 核心板一般可用 `GPIO16/GPIO17` 做 UART2。如果你的板子换了引脚，可在网页上修改 RX/TX GPIO。
...
如果没有配置 Wi-Fi，或连接失败，ESP32 会自动开热点：
...
板卡还没到时，可以先打开 [web/index.html](C:/Users/Lenovo/Documents/ESP32_UART/web/index.html) 核对页面功能。这个页面和 ESP32 固件内嵌页面来自同一份源码，直接从本地文件打开时会进入模拟模式：

- 模拟连接成功和串口接收数据
- 可以切换 ASCII / HEX 显示
- 可以修改波特率、数据位、校验位、停止位、RX/TX GPIO
- 可以测试 ASCII / HEX 发送、CR/LF、定时发送
- 可以测试多字符串条目、勾选顺序发送、循环发送、导入/导出 JSON
- 可以测试 UART BREAK，时间范围 `1..5000 ms`
...
网页本身走 `80` 端口，实时串口数据走 `81` 端口 WebSocket。如果页面能打开但一直显示未连接，优先检查 PC 防火墙、浏览器代理或局域网隔离。
