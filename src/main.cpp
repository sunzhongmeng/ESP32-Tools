...
void handleTextCommand(uint8_t clientIndex, const uint8_t *payload, size_t len) {
  ...
  if (command == "CLEAR_STATS") {
    ...
    uartTxBytes = 0;
    broadcastText(makeStatsMessage());
    sendWsText(wsClients[clientIndex], "INFO|message=Stats cleared");
    return;
  }
